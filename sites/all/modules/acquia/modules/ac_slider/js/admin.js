...
(function(e) {
	e(document).ready(function() {
		...
		var acquiaSlider = {
			...
			bindAsController : function(selector, container) {
				var controller = e('input[name="'+selector+'"]', e(container));
				controller.unbind().change(function(event) {
					if( e(this).prop('checked') == true ) {
						e('td[data-controller="'+selector+'"]', container).removeClass('disabled');
						e('td[data-controller="'+selector+'"] input, td[data-controller="'+selector+'"] select', container).prop('disabled', '');
						//e('input, select', wrap).prop("disabled","");
					} else {
						e('td[data-controller="'+selector+'"]', container).addClass('disabled');
						e('td[data-controller="'+selector+'"] input, td[data-controller="'+selector+'"] select', container).prop('disabled', 'disabled');
					}
				});

				if( e(controller).prop('checked') == true ) {
					e('td[data-controller="'+selector+'"]', container).removeClass('disabled');
					e('td[data-controller="'+selector+'"] input, td[data-controller="'+selector+'"] select', container).prop('disabled', '');
					//e('input, select', wrap).prop("disabled","");
				} else {
			...
			bindController : function(container) {
				e('td', e(container)).each(function(){
					if (ct = e(this).data('controller')) {
						acquiaSlider.bindAsController(ct, e(container));
			...
			newLayerbehaviours : function($layer) {
				// bind Controller
				acquiaSlider.bindController($layer);
				Drupal.attachBehaviors($layer);
			},
			...
			addLayer : function() {
				...
				// Attach behaviours
				acquiaSlider.newLayerbehaviours(clone);
			...
			play : function( index ) {
				...
				e('#ac-layers .ac-layer-box').each(function(layer) {
					...
					e(this).find('.ac-slide-options input, .ac-slide-options select, .ac-slide-options textarea').each(function() {
			...
			save : function(el) {
				...
				e("input[type=checkbox]").each(function() {
					if ( e(this).is(':checked') ) {
						e(this).prop('value', true);
					} else {
						e(this).prop('value', false);
					}
				});
				// Iterate over the settings
				e('.ac-settings input:not(.nochange), .ac-settings select').each(function() {
		...
		e(document).ready(function() {
			...
			acquiaSlider.addLayerSortables();
			acquiaSlider.addSortables();
			acquiaSlider.addDraggable();
			acquiaSlider.setAutoCompleteCaptions();
			...
			acquiaSlider.bindController('.ac-admin:not(.raw-attach)');
