...
if (is_array($slides)) {
  ...
  $data .= '<div id="' . $slide_id . '" style="width: ' . ac_check_unit($slides['properties']['width']) . '; height: ' . ac_check_unit($slides['properties']['height']) . '; margin: 0px auto; ' . $sliderStyle . '">';
                            ...
                            if(isset($layer['sublayers']) && is_array($layer['sublayers'])) {
                                foreach($layer['sublayers'] as $sublayer) {
                                    ...
                                    if(!empty($sublayer['url']) && preg_match('/^\#[0-9]/', $sublayer['url']) == 0) {
                                        $data .= '<a href="'.$sublayer['url'].'" target="'.$sublayer['target'].'" '.$sublayerID.' '.$sublayerRel.' class="ls-s'.$sublayer['level'].'" '.$sublayerTitle.' style="position: absolute; top: '.ac_check_unit($sublayer['top']).'; left:'.ac_check_unit($sublayer['left']).'; display: block; '.$slidedirection.' '.$slideoutdirection.' durationin : '.$sublayer['durationin'].'; durationout : '.$sublayer['durationout'].'; easingin : '.$sublayer['easingin'].'; easingout : '.$sublayer['easingout'].'; delayin : '.$sublayer['delayin'].'; delayout : '.$sublayer['delayout'].'; '.$sublayer['rotatein'].''.$sublayer['rotateout'].''.$sublayer['scalein'].''.$sublayer['scaleout'].'showuntil : '.$sublayer['showuntil'].'">';
                                        ...
                                        if(empty($sublayer['type']) || $sublayer['type'] == 'img') {
                                            if(!empty($sublayer['image'])) {
                                                $data .= '<img class="ls-s'.$sublayer['level'].''.$linkTo.''.$sublayerClass.'" '.$sublayerID.' src="'.$sublayer['image'].'" '.$sublayerAlt.' style="position: absolute; top: '.ac_check_unit($sublayer['top']).'; left: '.ac_check_unit($sublayer['left']).'; '.$slidedirection.' '.$slideoutdirection.'  durationin : '.$sublayer['durationin'].'; durationout : '.$sublayer['durationout'].'; easingin : '.$sublayer['easingin'].'; easingout : '.$sublayer['easingout'].'; delayin : '.$sublayer['delayin'].'; delayout : '.$sublayer['delayout'].'; '.$sublayer['rotatein'].''.$sublayer['rotateout'].''.$sublayer['scalein'].''.$sublayer['scaleout'].'showuntil : '.$sublayer['showuntil'].'; '.$sublayerStyle.''.$customStyles.'">';
                                            }
                                        } else {
                                          if (in_array($sublayer ['type'], array('p', 'div'))) {
                                            $sublayer ['html'] = check_markup($sublayer ['html'], 'full_html');
                                          }
                                          $data .= '<'.$sublayer['type'].' '.$sublayerID.' class="ls-s'.$sublayer['level'].''.$linkTo.''.$sublayerClass.'" style="position: absolute; top:'.ac_check_unit($sublayer['top']).'; left: '.ac_check_unit($sublayer['left']).'; '.$slidedirection.' '.$slideoutdirection.' durationin : '.$sublayer['durationin'].'; durationout : '.$sublayer['durationout'].'; easingin : '.$sublayer['easingin'].'; easingout : '.$sublayer['easingout'].'; delayin : '.$sublayer['delayin'].'; delayout : '.$sublayer['delayout'].'; '.$sublayer['rotatein'].''.$sublayer['rotateout'].''.$sublayer['scalein'].''.$sublayer['scaleout'].'showuntil : '.$sublayer['showuntil'].'; '.$sublayerStyle.' '.$customStyles.''.$sublayerWordWrap.'"> '.$sublayer['html'].' </'.$sublayer['type'].'>';
                                        }
                                    }
                                }
                            }
