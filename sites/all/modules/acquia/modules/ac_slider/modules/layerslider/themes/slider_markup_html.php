...
// Get slider style
$sliderStyleAttr[] = 'width:'.ac_check_unit($slides['properties']['props']['width']).';';
$sliderStyleAttr[] = 'height:'.ac_check_unit($slides['properties']['props']['height']).';';

if(!empty($slides['properties']['props']['maxwidth'])) {
	$sliderStyleAttr[] = 'max-width:'.ac_check_unit($slides['properties']['props']['maxwidth']).';';
}
...
if(!empty($slider['slides']) && is_array($slider['slides'])) {
	foreach($slider['slides'] as $slidekey => $slide) {
		...
		if(!empty($slide['props']['background'])) {
			$src = !empty($slide['props']['backgroundId']) ? apply_filters('ls_get_image', (int)$slide['props']['backgroundId'], (string)$slide['props']['background']) : $slide['props']['background'];
			$data[] = '<img src="'.acquia_variable_get('layerslider_root').'/static/img/blank.gif" data-src="'.$src.'" class="ls-bg" alt="Slide background" />';
		}
		...
		if(isset($slides['properties']['attrs']['thumbnailNavigation'])) {
			if($slides['properties']['attrs']['thumbnailNavigation'] != 'disabled') {
				if(!empty($slide['props']['thumbnail'])) {
					$src = !empty($slide['props']['thumbnailId']) ? apply_filters('ls_get_image', (int)$slide['props']['thumbnailId'], (string)$slide['props']['thumbnail']) : $slide['props']['thumbnail'];
					$data[] = '<img src="'.acquia_variable_get('layerslider_root').'/static/img/blank.gif" data-src="'.$src.'" class="ls-tn" alt="Slide thumbnail" />';
				}
			}
		}
		...
		if(!empty($slide['layers']) && is_array($slide['layers'])) {
			foreach($slide['layers'] as $layerkey => $layer) {
				...
				// Transition & style
				$el->attr('style', 'top:'.ac_check_unit($layer['props']['top']).';');
				$el->attr('style', $el->attr('style') . 'left:'.ac_check_unit($layer['props']['left']).';');
				...
				if($layer['props']['type'] == 'img') {
					$inner->attr('src', acquia_variable_get('layerslider_root').'/static/img/blank.gif');
					$inner->attr('src', acquia_variable_get('layerslider_root').'/static/img/blank.gif');
				...
				// Text / HTML layer
					...
					$inner->html(check_markup(__(stripslashes($layer['props']['html'])), 'full_html'));
				}
