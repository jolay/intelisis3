...
(function( e ) {
	...
	e.fn.customCheckbox = function() {
		...
		return this.each(function() {
			...
			var rep = e('<a href="#"><span></span></a>').addClass('ls-checkbox').insertAfter(this);

			// Check help attr
			if(e(this).attr('data-help') != "undefined") {
				e(rep).attr('data-help', e(this).attr('data-help'));
			}
  ...
  var drupal_base_path = Drupal.settings.basePath;
		...
		openMediaLibrary : function() {
			...
			$('.ls-upload').live('click', function() {
				...
				Drupal.media.popups.mediaBrowser(function(t) {
					...
					// Generate preview
					$('#ls-layers .ls-layer-box').each(function( index ) {
						LayerSlider.generatePreview(index);
					});
					
					if($(uploadInput).attr('name') == 'image'){
					...
					// Image sublayer
					if($(uploadInput).is('input[name="image"]')) {
						$(uploadInput).prev().attr('src', mediaFile.url);
					}
	...
	addLayerSortables : function() {
		...
		play : function( index ) {
			...
			var skin = jQuery('.ls-settings select[name="skin"]').val();
			var thumb_nav = jQuery('.ls-settings select[name="thumb_nav"]').val();
		...
		save : function(el) {
			...
			jQuery('.ls-saving-warning').text('Please do not navigate away from this page while layerslider saving your layers!');
			...
			setTimeout(function() {
				...
				$data = jQuery('#ls-slider-form > input');
				// Iterate over the layers
				jQuery('#ls-layers .ls-layer-box').each(function(layer) {
...
// List view
	...
	// Editor view
		...
		$('input, select, textarea, .ls-checkbox').each(function(){
			if (typeof($(this).attr('data-help')) !='undefined' && $(this).attr('data-help') !='') {
				$(this).addClass('tooltip').acquiaTooltip();
			}
		});
		
		$('.ls-colorpicker').acquiaColorPicker();
		...
		// Duplicate layer
		jQuery('#ls-layers').on('click', '.ls-layer-options-thead .duplicate', function(e){
			e.preventDefault();
