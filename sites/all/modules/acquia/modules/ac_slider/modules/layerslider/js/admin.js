		...
		openMediaLibrary : function() {
			...
			jQuery(document).on('click', '.ls-upload', function(e) {
				...
				Drupal.media.popups.mediaBrowser(function(t) {
					...
					if(jQuery(uploadInput).attr('name') == 'image'){
						jQuery('.ls-sublayer-pages .ls-sublayer-page').each(function(index){
							tmp_ls = jQuery(this);
					...
					if(jQuery(uploadInput).is('input[name="image"]')) {
						jQuery(uploadInput).prev().attr('src', mediaFile.url);
					}
					
					if( jQuery(uploadInput).is('div.ls-image') ) {
						jQuery(uploadInput).prev().val(attachment.url);
						jQuery(uploadInput).prev().prev().val(attachment.fid);
						if(typeof attachment.preview != "undefined") {
							var temp = jQuery(attachment.preview);
							jQuery(uploadInput).find('img').attr('src', jQuery(temp).find('img').attr('src'));
						} else {
							jQuery(uploadInput).find('img').attr('src', attachment.url);
						}
