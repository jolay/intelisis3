...
$slides['properties']['attrs']['skinsPath'] = acquia_variable_get('layerslider_root').'/static/skins/';
...
$data[] = '</script>';
	...
	if(variable_get('ls_put_js_to_body', false)) {
			$data[] = '<script type="text/javascript" src="'.acquia_variable_get('layerslider_root').'/static/js/layerslider.kreaturamedia.jquery.js?ver='.LS_PLUGIN_VERSION.'"></script>' . NL;
			$data[] = '<script type="text/javascript" src="'.acquia_variable_get('layerslider_root').'/static/js/greensock.js?ver=1.11.2"></script>' . NL;
	}

$data[] = '<script type="text/javascript">' . NL;
	$data[] = 'lsjQuery(document).ready(function() {' . NL;
		$data[] = 'if(typeof lsjQuery.fn.layerSlider == "undefined") {  }' . NL;
		$data[] = 'else {' . NL;
