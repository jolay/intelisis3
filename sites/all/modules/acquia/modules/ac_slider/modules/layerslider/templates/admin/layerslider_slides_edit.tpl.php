	...
	<div class="ls-box ls-layer-box">
		...
		<table>
          ...
          <td colspan="7">
            ...
            <button class="button duplicate ls-layer-duplicate"><?php _e('Duplicate this slide', 'LayerSlider') ?></button>
          </td>
				</tr>
			</thead>
...
<form action="<?php echo $_SERVER['REQUEST_URI'] ?>" method="post" class="wrap" id="ls-slider-form">
  ...
  <input type="hidden" name="slider_edit" value="1">
	...
	<h2>
		...
		<a href="<?php print url(SLIDER_LIST_PATH)?>" class="add-new-h2"><?php _e('Back to the list', 'LayerSlider') ?></a>
	</h2>
	...
	<div id="ls-main-nav-bar">
		...
		<?php print help_layerslider_index_page() ?>
	...
	<div id="ls-pages">
		...
		<div class="ls-page ls-settings">
			...
			<div class="ls-box" id="global-settings">
				...
				<table>
					...
					<tbody>
						...
						<tr>
							<td><?php _e('Disable Title?', 'LayerSlider') ?></td>
							<td><input type="checkbox" name="disable_title" <?php echo isset($slider['properties']['disable_title']) ? 'checked="checked"' : '' ?>></td>
							<td class="desc"><?php _e('You can disable title by turning on this option.', 'LayerSlider') ?></td>
						</tr>
					</tbody>
					<thead>
						<tr>
					...
					<tbody>
						...
						<tr>
							...
							<td><input type="text" name="firstlayer" value="<?php echo isset($slider['properties']['firstlayer']) ? $slider['properties']['firstlayer'] : 1?>" class="input"></td>
					...
					<tbody>
						<tr>
							...
							<td>
								<select name="skin">
									<?php foreach($skins as $mn => $skin) : ?>
									<?php if($mn == $slider['properties']['skin']) { ?>
									<option value="<?php print $mn?>" selected="selected"><?php echo $skin ?></option>
									<?php } else { ?>
									<option value="<?php print $mn?>" ><?php echo $skin ?></option>
									<?php } ?>
									<?php endforeach; ?>
								</select>
							</td>
    ...
    <!-- Event Callbacks -->
		<div class="ls-page ls-callback-page">
			<div class="ls-box ls-callback-box">
				...
				<div class="inner">
					<textarea name="cbinit" cols="20" rows="5"><?php echo !empty($slider['properties']['cbinit']) ? stripslashes($slider['properties']['cbinit']) : 'function(element) { }' ?></textarea>
				</div>
			</div>
