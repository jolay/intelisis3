...
(function ($) {
    ...
    var ShortcodeView = vc.shortcode_view = Backbone.View.extend({
        ...
        events:{
            ...
            'click .column_toggle':'toggleElement',
        },
        removeView:function () {
            this.remove();
        },
        ...
        clone:function (e) {
            ...
            if (_.isObject(e)) e.preventDefault();
						...
						this.$el.find('.wpb_element_wrapper:first').toggle();
            ...
            if (model.get('parent_id')) {
                var view = vc.app.views[model.get('parent_id')];
    ...
    var ElementBlockView = vc.element_block_view = Backbone.View.extend({
        ...
        data_saved:false,
        events:{
            'click .wpb-layout-element-button':'createElement',
            'click .close':'close',
            'hidden':'removeView',
            'shown':'setupShown',
        ...
        render:function () {
            var that = this,
                ...
                tag,
                not_in;
            $('body').append(this.$el.html($('#wpb-elements-list-modal-template').html()));
    ...
    var SettingsView = Backbone.View.extend({
        ...
        initialize:function () {
						Drupal.acquia.composer.currentModelView = this;
						// Build Modal
						var settings = Drupal.settings.acquiaModalStyle,
						tag = this.model.get('shortcode'),
						tagInfo = _.isObject(vc.map[tag]) ? vc.map[tag] : [],
						modal_data = _.isObject(tagInfo.modal_data) ? tagInfo.modal_data : [];

						if (modal_data.modal_size != '' && modal_data.modal_size != undefined) {
							var fact = acquia_get_modal_size_factor(modal_data.modal_size);
							settings.modalSize.width = fact.width;
							settings.modalSize.height = fact.height;
						}
						settings.modalSaveBtn = true;
						this.modal = Drupal.acquia.Modal.show(settings);
						this.$el = this.modal.modal;
        ...
        render:function () {
						...
						ajax.submit.action = 'wpb_show_edit_form';
						ajax.submit.element = this.model.get('shortcode');
						ajax.submit.shortcode = this.model.get('params');
						if (!_.isUndefined(ajax.submit.shortcode.content)) {
						debug.log(this.model);
						debug.log(ajax.submit.shortcode.content);
							//ajax.submit.shortcode.content = window.nl2br(ajax.submit.shortcode.content);
						}
						...
						if ( !_.isUndefined(settings['nested shortcodes']) && _.isObject(settings['nested shortcodes']) ) {
							...
							_.each(settings['nested shortcodes'], function(val, key){
								var nested = Shortcodes.where({parent_id:model_ID});
    ...
    var VisualComposer = Backbone.View.extend({
        ...
        render:function () {
            ...
            this.mceID = this.$el.parent('#poststuff').next('.form-type-textarea').find('textarea.form-textarea').attr('id');
						Drupal.acquia.composer.composerEditorID = this.mceID;
