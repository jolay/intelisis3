	 ...
	 window.VcTabsView = vc.shortcode_view.extend({
        ...
        addTab:function (e) {
            ...
            var tab_title = this.model.get('shortcode') === 'vc_tour' ? window.i18nLocale.slide : window.i18nLocale.tab,
                tabs_count = this.$tabs.find('[data-element_type=ac_tab]').length,
                tab_id = (+new Date() + '-' + tabs_count + '-' + Math.floor(Math.random() * 11));
            vc.shortcodes.create({shortcode:'ac_tab', params:{title:tab_title, tab_id:tab_id}, parent_id:this.model.id});
            return false;
        },
        stopSorting:function (event, ui) {
            ...
            this.$tabs.find('ul.tabs_controls li:not(.add_tab_block)').each(function (index) {
                ...
                shortcode = vc.shortcodes.get($( $('a', this).attr('href') ).data('model-id'));
                vc.storage.lock();
                shortcode.save({'order':$(this).index()}); // Optimize
            });
            shortcode.save();
        ...
        cloneModel:function (model, parent_id, save_order) {
            ...
            model_clone = Shortcodes.create({shortcode:model.get('shortcode'), id:vc_guid(), parent_id:parent_id, order:new_order, cloned:(model.get('shortcode') === 'ac_tab' ? false : true), cloned_from:model.toJSON(), params:new_params});
    ...
    window.VcTabView = window.VcColumnView.extend({
        ...
        cloneModel:function (model, parent_id, save_order) {
            var shortcodes_to_resort = [],
                new_order = _.isBoolean(save_order) && save_order === true ? model.get('order') : parseFloat(model.get('order')) + vc.clone_index,
                new_params = _.extend({}, model.get('params'));
            if (model.get('shortcode') === 'ac_tab') _.extend(new_params, {tab_id:+new Date() + '-' + this.$tabs.find('[data-element_type=ac_tab]').length + '-' + Math.floor(Math.random() * 11)});
        ...
        addTab:function (e) {
				   e.preventDefault();
						settings = vc.map[this.model.get('shortcode')];
						var cloneTag = !_.isUndefined(settings['clone_tag']) ? settings['clone_tag'] : this.model.get('shortcode');
            this.adding_new_tab = true;
            vc.shortcodes.create({shortcode:cloneTag, params:{title:window.i18nLocale.title}, parent_id:this.model.id});
        },
		});

		/** Custom Acquia Shortcode View **/
		window.acquiaShortcodeView = vc.shortcode_view.extend({
				editElement:function (e) {
					if (_.isObject(e)) e.preventDefault();
            var params = this.model.get('params'),
						settings = vc.map[this.model.get('shortcode')];
					...
					window.acquiaShortcodeView.__super__.editElement.call(this);
				},
        changeShortcodeParams:function (model) {
            var params = model.get('params'),
							...
							_.each(settings['nested shortcodes'], function(val, key){
								if (params[key] != "undefined") {
									_.each(params[key], function(n_params){
										// Build a new Column
										Shortcodes.create({shortcode: val,  id:vc_guid(), params:n_params, parent_id:model_ID});
									});
									delete params[key];
								}
							});
						...
						visual_updates.each(function(){
							visual_el	= $(this);
							visual_key	= visual_el.attr('data-update_with');
            ...
            window.acquiaShortcodeView.__super__.changeShortcodeParams.call(this, model);
        },
    });
		
		/** Image Shortcode Composer View **/
		window.acquiaImageView = window.acquiaShortcodeView.extend({
        changeShortcodeParams:function (model) {
				

            var params = model.get('params'),
						that = this;
						if (!_.isUndefined(params.fid)) {
							   $.ajax({
                        type:'POST',
                        url:window.ajaxurl,
                        data:{
                            action:'wpb_get_img_src',
                            fid:params.fid
                        },
                        dataType:'html',
                        context:this
												...
												params.src = src;
												window.acquiaImageView.__super__.changeShortcodeParams.call(that, model);
                    });
	
						}else if (!_.isUndefined(params.src)) {
							window.acquiaImageView.__super__.changeShortcodeParams.call(that, model);
						}
            window.acquiaImageView.__super__.changeShortcodeParams.call(that, model);
        },
		...
		window.acIconView = window.acquiaShortcodeView.extend({
        changeShortcodeParams:function (model) {
            var params = model.get('params'),
						that = this;
                        ...
                        url:window.ajaxurl,
                        data:{
                            action:'wpb_get_icon_str',
                            icon:params.icon
                        },
                        dataType:'html',
                        context:this
                    }).done(function (icon_str) {
												params.icon_str = icon_str;
												window.acIconView.__super__.changeShortcodeParams.call(that, model);
                    });
		...
		window.AcprogressbarItemView = window.acquiaShortcodeView.extend({
			editElement:function (e) {
        if (_.isObject(e)) e.preventDefault();
				var parent = Shortcodes.where({id:this.model.get('parent_id')});
				parentParams = parent[0].get('params'),
